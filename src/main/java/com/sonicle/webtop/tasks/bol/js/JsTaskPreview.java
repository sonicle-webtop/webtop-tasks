...
public class JsTaskPreview {
	...
	public JsTaskPreview(CategoryFSOrigin origin, CategoryFSFolder folder, CategoryPropSet folderProps, TaskInstance item, Collection<CustomPanel> customPanels, Map<String, CustomField> customFields, String profileLanguageTag, DateTimeZone profileTz) {
		...
		this.start = !isSeriesMaster ? JodaTimeUtils.printYMDHMS(profileTz, item.getStart()) : null;
		this.due = !isSeriesMaster ? JodaTimeUtils.printYMDHMS(profileTz, item.getDue()) : null;
		this.completedOn = JodaTimeUtils.printYMDHMS(profileTz, item.getCompletedOn());
